...
if (!empty($data->nome) && !empty($data->carro) && !empty($data->telefone)) {
    // Preparar instrução SQL para inserir o passageiro
    $query = "INSERT INTO drivers (nome, carro, telefone) VALUES (:nome, :carro, :telefone)";
    $stmt = $db->prepare($query);

    // Limpar dados
    $nome = htmlspecialchars(strip_tags($data->nome));
    $carro = htmlspecialchars(strip_tags($data->carro));
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(["message" => "Não foi possível cadastrar o motorista."]);
    }
} else {
